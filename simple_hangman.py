...
print("")
easyWords = [
    "shadowofthecolossus", "leagueoflegends", "grandtheftauto",
    "supersmashbros", "legendofzelda", "supermarioodyssey", "sonicthehedgehog",
    "streetfighter", "residentevil", "undertale"
]
hardWords = [
    "fortnite", "minecraft", "pubg", "cuphead", "octodad", "godofwar",
    "titanfall", "fallout", "overwatch", "darksouls", "skyrim", "splatoon",
    "callofduty", "pokemon", "thesims", "pacman", "tetris", "doom"
]


def isAlpha(char):
    if (char == "a" or char == "b" or char == "c" or char == "d" or char == "e"
            or char == "f" or char == "g" or char == "h" or char == "i"
            or char == "j" or char == "k" or char == "l" or char == "m"
            or char == "n" or char == "o" or char == "p" or char == "q"
            or char == "r" or char == "s" or char == "t" or char == "u"
            or char == "v" or char == "w" or char == "x" or char == "y"
            or char == "z"):
        return True
...
while (selectedDifficulty != True):
    ...
    if (press == "lootbox"):
        ...
        if (item == 16):
            print(
                "you got burned. You feel incredibly insulted and your self confidence is down the drain"
            )
        if (item == 17):
            print("you got a new understanding of the furry fandom")
        if (item == 18):
            print(
                "you got a bag of various girlscout cookies but none of the flavor you like"
            )
        if (item == 19):
            print("you got in trouble. Go to jaaiiiillll")
        if (item == 20):
            print(
                "you got Todd Howard on your doorstep coming to take away your computer"
            )
    if (item == 21):
        print("You got dead")
currentWord = ""
...
while (lives == 0 or victory != True):
    ...
    if (cheatMode == True):
        print(currentWord)
    guessCorrect = False
    print("The word is " + str(length) + " letters long. [" + str(lives) +
          " lives left]")
    print(*wordBlank)
    ...
    if (isAlpha(text) and len(text) == 1):
        ...
        print(
            '\033[1m' +
            "Invalid Response. Be sure your input is a letter and only one character long"
        )
        guessCorrect = True
    if (victory == True):
    ...
    if (guessCorrect == False):
        if (usedAgain != True):
            lives -= 1
            print('\033[1m' + "There are no " + text +
                  "\'s in the word. Try again [-1 life]")
    if (lives == 0):
